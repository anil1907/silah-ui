...
import category, { getCategories, selectedCategory } from 'store/slices/category';
...
const CategoryPage = () => {
    ...
    const [rows, setRows] = React.useState(categories?.items || []);
    ...
    React.useEffect(() => {
        if (categories?.index === 0) {
            ...
            setRows((prevRows) => [...prevRows, ...(categories?.items || [])]);
        }
        setPage(categories?.index || 0);
        setRowsPerPage(categories?.size || 5);
        setCount(categories?.count || 0);
    }, [categories]);

    React.useEffect(() => {
        dispatch(getCategories(page, rowsPerPage));
    ...
    const handleChangePage = (event, newPage) => {
        setPage(newPage);
        dispatch(getCategories(newPage, rowsPerPage));
    };

    const handleChangeRowsPerPage = (event) => {
        setRowsPerPage(parseInt(event?.target.value, 10));
        setPage(0);
        dispatch(getCategories(page, parseInt(event?.target.value, 10)));
    };
            ...
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} sx={{ textAlign: 'right' }}>
                        ...
                        <CategoryAdd open={open} handleCloseDialog={handleCloseDialog} />
                    </Grid>
                </Grid>
            </CardContent>
